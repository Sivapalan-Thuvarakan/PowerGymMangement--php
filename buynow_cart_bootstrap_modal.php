...
<?php
  ...
  {
    $sq="insert into `order` (Customer_ID,Province,District,Cardtype,Phone_no,Zip,Status,Shipping_address)
    values({$_SESSION['UID']},'{$_POST['province']}','{$_POST['District']}','{$_POST['cardtype']}',
    {$_POST['phone']},{$_POST['zip']},'order ready','{$_POST['address']}')";
    $db->query($sq);
    ...
    foreach($_SESSION["cart"] as $keys=>$values)
      {
          ...
          $product=$values["pname"];
          $address=$_POST['address'];
            $amt=$values["qty"]*$values["price"];
            $sq3="INSERT INTO `order_item`(`Order_Id`, `Product_Id`,`P_NAME`,`Price`,`Quantity`, `subtotal`) VALUES ({$last_order_id},
            {$values['pid']},'{$product}',{$Payment},{$values['qty']},{$amt})";
            $db->query($sq3);
       ...
       <script> 
        ...
        <div class="row">
          ...
          <div class="col-md-6 mb-3">
            ...
            <input type="text" class="form-control" id="cc-number" placeholder="" value= required>
            <div class="invalid-feedback">
